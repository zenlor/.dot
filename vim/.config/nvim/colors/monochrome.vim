...
if &background == "dark"
    let s:default_fg = s:dgray
    let s:default_bg = s:black
else
    let s:default_fg = s:bgray
    let s:default_bg = s:llgray
endif
...
call s:hi('Statement', ( &background == "dark"? s:white : s:black ), s:default_bg, s:bold)
call s:hi('PreProc', ( &background == "dark"? s:white : s:black ), s:default_bg, s:bold)
